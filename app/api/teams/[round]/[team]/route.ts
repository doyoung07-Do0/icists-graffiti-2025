...
export async function POST(
  ...
  try {
    ...
    if (result) {
      try {
        console.log(
          `[${new Date().toISOString()}] Broadcasting team submission: ${team} submitted for round ${round}`,
        );

        // Import the SSE broadcasting function
        const { sendAdminUpdate } = await import('@/lib/sse');

        // Send update to admin clients only
        sendAdminUpdate(team, round, {
          type: 'team_submitted',
          team: team,
          round: round,
          data: result,
        ...
        console.log(
          `[${new Date().toISOString()}] Team submission broadcast completed`,
        );
      } catch (error) {
        console.error(
          `[${new Date().toISOString()}] Error broadcasting team submission:`,
          error,
        );
      }
    } else {
